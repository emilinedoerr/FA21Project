...
- The sample IDs, classification (healthy/unhealthy), and miRNA IDs are provided in the supplemental information from Rovira et al. and translated into a csv file to create a design matrix for the samples. 
...
```{r heatmap}
expr <- t(exprs(mature.eset))
rownames(expr) <- paste(gsub("serum_","",mature.eset$source_name_ch1),
                        gsub("CB","",mature.eset$title),
                        sep="")

install.packages("pheatmap")
library("pheatmap")

annotation <- pData(mature.eset)["group:ch1"]
rownames(annotation) <- labels

pheatmap(expr, scale="row", cluster_rows=TRUE, cutree_rows = 2, annotation_row = annotation)
