...
mirbase.query.up <- get_multimir(org = 'hsa', mirna = upreg$miRNA_ID, 
                                table = 'mirtarbase', 
                                predicted.cutoff = 20,
...
#index <- match(upreg$miRNA_ID, mirbase.data.up$mature_mirna_id)
...
# Extract all target genes for each
up.5.data <- subset(mirbase.data.up, mirbase.data.up$mature_mirna_id%in%up.5$miRNA_ID)

x<-up.5$miRNA_ID[1]
x.targs <- up.5.data[up.5.data$mature_mirna_id==x,]
#x.targs <- up.5.data[up.5.data$mature_mirna_id==x,c("target_symbol")]

x.targs
unique(x.targs$target_symbol)
freq <- as.data.frame(table(x.targs$target_symbol))
